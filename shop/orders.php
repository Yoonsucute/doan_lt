...
<main class="container py-5">
    ...
    <?php if (!$orders) { ?>
        <div class="shop-empty">Bạn chưa có đơn hàng nào.</div>
    <?php } ?>
    ...
    <?php foreach ($orders as $order) { ?>
        <div class="card p-4 mb-4">
            <div class="d-flex flex-column flex-md-row justify-content-between gap-2 mb-3">
                <div>
                    <h4 class="mb-1">Đơn hàng #<?php echo (int) $order['id']; ?></h4>
                    <div class="text-muted"><?php echo e($order['created_at']); ?></div>
                </div>
                <div class="text-md-end">
                    <span class="badge text-bg-<?php echo in_array($order['status'], ['paid', 'completed'], true) ? 'success' : 'warning'; ?>"><?php echo e($order['status']); ?></span>
                    <div class="fw-bold mt-1"><?php echo money($order['total']); ?></div>
                </div>
            </div>
            <div class="table-responsive">
                <table class="table">
                    <tr><th>Source</th><th>Giá</th><th>SL</th><th>Tai file</th></tr>
